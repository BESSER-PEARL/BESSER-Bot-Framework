...
from besser.bot.platforms.websocket.streamlit_ui.vars import SESSION_MONITORING_INTERVAL, SUBMIT_TEXT, HISTORY, QUEUE, \
    WEBSOCKET, SESSION_MONITORING, SUBMIT_AUDIO, SUBMIT_FILE, VIDEO_INPUT_ENABLED, VIDEO_INPUT_INTERVAL
...
def initialize():
    if VIDEO_INPUT_ENABLED not in st.session_state:
        st.session_state[VIDEO_INPUT_ENABLED] = False

    if VIDEO_INPUT_INTERVAL not in st.session_state:
        st.session_state[VIDEO_INPUT_INTERVAL] = 0.2
