...
_SESSION_RAG_KEY = '__session_rag__'
...
class Session:
    ...
    @property
    def session_rag(self) -> RAG:
        """RAG: The session-scoped RAG for this session, or ``None`` if not configured."""
        return self.__dict__.get(_SESSION_RAG_KEY)

    @session_rag.setter
    def session_rag(self, rag: RAG) -> None:
        """Set the session-scoped RAG for this session.

        Args:
            rag (RAG): a :class:`~baf.nlp.rag.rag.RAG` instance created with ``session_scoped=True``
        """
        # Stored as a plain instance attribute, not in _dictionary, so it is
        # never picked up by get_dictionary() / JSON serialization to monitoring DB.
        # RAG holds live in-memory objects (Chroma client, embeddings) that cannot
        # be meaningfully serialized or restored from JSON.
        self.__dict__[_SESSION_RAG_KEY] = rag

    def run_session_rag(self, message: str, llm_prompt: str = None, llm_name: str = None, k: int = None) -> RAGMessage:
        """Run RAG over this session's isolated corpus.

        Args:
            message (str): the input query
            llm_prompt (str): override the LLM prompt for this call
            llm_name (str): override the LLM name for this call
            k (int): override the number of chunks to retrieve for this call

        Returns:
            RAGMessage: the answer generated by the session-scoped RAG engine

        Raises:
            RuntimeError: if no session-scoped RAG has been assigned to this session
        """
        if self.session_rag is None:
            raise RuntimeError('No session-scoped RAG configured. Assign one via session.session_rag = RAG(..., session_scoped=True).')
        return self.session_rag.run(message, self, llm_prompt, llm_name, k)